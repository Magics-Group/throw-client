    ...
    render() {
        ...
        return (
            <div className={'control-bar ' + (this.props.uiShown ? 'show' : null)} >
    			...
    			<IconButton onClick={() => this.props.emitter.emit('togglePause')} iconClassName="material-icons" iconStyle={{top: '-5px', left: '-1px'}} className={this.state.rippleEffects ? 'play-toggle' : 'play-toggle no-ripples'}>{this.state.playing ? 'pause' : 'play_arrow'}</IconButton>
